...
import { isTenantConnectionBroken, tenantConnectionError } from "@/shared/utils";
...
export function RealmsPage() {
  ...
  const { data: keycloakDashboard, isLoading, isError } = useKeycloakDashboard({
  ...
  const connectionBroken =
    isTenantConnectionBroken(selectedTenant) || (isError && allRealms.length === 0);
