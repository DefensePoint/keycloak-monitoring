...
| Geolocation map tiles show "API KEY REQUIRED" watermarked across them | CARTO's raster basemaps now require an API key. Without one the CDN still answers `200` with a valid PNG, so every network request looks healthy while the map looks broken. | Request a free key at <https://carto.com/basemaps/apikey/> — email, the domain you serve from, and roughly what you are building; it is emailed straight back with no approval queue and no CARTO account. Free up to a fair use limit of 5 million tile requests per calendar month, commercial use included. Supply it as a build arg when building the web image: `docker build --build-arg SERVICE=web --build-arg VITE_CARTO_BASEMAP_KEY=<key> -f build/Dockerfile .` — Vite inlines env vars at build time, so it cannot be set at runtime. Keep the CARTO and OpenStreetMap attribution visible — that is the stated condition of the free tier. CARTO describe the raster tiles as "being retired" in favour of vector styles, with no timeline, so plan to revisit. |
